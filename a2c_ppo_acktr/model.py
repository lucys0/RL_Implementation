...
def random_translate(imgs, size, return_random_idxs=False, h1s=None, w1s=None):
    ...
    outs = torch.empty((n, c, size, size), dtype=imgs.dtype, device=imgs.device)
    h1s = torch.randint(0, size - h + 1, (n,)) if h1s is None else h1s
    w1s = torch.randint(0, size - w + 1, (n,)) if w1s is None else w1s
...
class CNNBase(NNBase):
    def __init__(self, num_inputs, trained_encoder=None, recurrent=False, hidden_size=512):
    ...
    def forward(self, inputs, rnn_hxs, masks):
        ...
        # inputs = random_translate(inputs, 64)
