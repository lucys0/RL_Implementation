...
def main():
    args = get_args()
    log_dir = 'runs/env=' + args.env_name + '_lr=' + str(args.lr) + ' ||' + time.strftime("%d-%m-%Y_%H-%M-%S")
    ...
    #             model, batch, optimizer, decoder_optimizer)
    #         running_loss += loss
    ...
    vid_rollouts = RolloutStorage(32, args.num_processes,
                              envs.observation_space.shape, envs.action_space,
                              actor_critic.recurrent_hidden_state_size)
    ...
    for j in range(num_updates):
        ...
        if log_video:
            # perform logging
            # for step in range(32):  # num steps for video collection
            #     with torch.no_grad():
            #         value, action, action_log_prob, recurrent_hidden_states = actor_critic.act(
